...
#include "KSYPlotCircularBuffer.h"
#include <mach/mach.h>
#include <stdio.h>
#include <stdlib.h>
...
bool _KSYPlotCircularBufferInit(KSYPlotCircularBuffer *buffer, int32_t length, size_t structSize) {
    ...
    if ( structSize != sizeof(KSYPlotCircularBuffer) ) {
        fprintf(stderr, "KSYPlotCircularBuffer: Header version mismatch. Check for old versions of KSYPlotCircularBuffer in your project\n");
        abort();
    }
...
void KSYPlotCircularBufferCleanup(KSYPlotCircularBuffer *buffer) {
    ...
    memset(buffer, 0, sizeof(KSYPlotCircularBuffer));
}

void KSYPlotCircularBufferClear(KSYPlotCircularBuffer *buffer) {
    int32_t fillCount;
    if ( KSYPlotCircularBufferTail(buffer, &fillCount) ) {
        KSYPlotCircularBufferConsume(buffer, fillCount);
    }
}
