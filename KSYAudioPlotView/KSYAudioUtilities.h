...
typedef struct
{
    float            *buffer;
    int               bufferSize;
    KSYPlotCircularBuffer  circularBuffer;
} KSYPlotHistoryInfo;
...
/**
 Given a buffer representing a window of float history data this append the RMS of a buffer of incoming float data...This will likely be deprecated in a future version of KSYAudio for a circular buffer based approach.
...
///-----------------------------------------------------------
/// @name KSYPlotCircularBuffer Utility
///-----------------------------------------------------------

/**
 Appends the data from the audio buffer list to the circular buffer
 @param circularBuffer  Pointer to the instance of the KSYPlotCircularBuffer to add the audio data to
...
/**
 ...
 @param circularBuffer Pointer to an instance of the KSYPlotCircularBuffer
 @param size           The length of the KSYPlotCircularBuffer (usually 1024)
 */
+ (void)circularBuffer:(KSYPlotCircularBuffer*)circularBuffer
              withSize:(int)size;
...
/**
 Calculates the RMS of a float array containing audio data and appends it to the tail of a KSYPlotHistoryInfo data structure. Thread-safe.
 ...
 @param historyInfo A pointer to a KSYPlotHistoryInfo structure to use for managing the history buffers
 */
...
/**
 Appends a buffer of audio data to the tail of a KSYPlotHistoryInfo data structure. Thread-safe.
 @param buffer      A float array containing the incoming audio buffer to append to the history buffer
 @param bufferSize  A UInt32 representing the length of the incoming audio buffer
 @param historyInfo A pointer to a KSYPlotHistoryInfo structure to use for managing the history buffers
 */
...
/**
 Zeroes out a KSYPlotHistoryInfo data structure without freeing the resources.
 @param historyInfo A pointer to a KSYPlotHistoryInfo data structure
 */
...
/**
 Frees a KSYPlotHistoryInfo data structure
 @param historyInfo A pointer to a KSYPlotHistoryInfo data structure
 */
...
/**
 Creates an KSYPlotHistoryInfo data structure with a default length for the window buffer and a maximum length capacity for the internal circular buffer that holds all the audio data.
 ...
 @return A pointer to the KSYPlotHistoryInfo created. The caller is responsible for freeing this structure using the `freeHistoryInfo` method above.
 */
